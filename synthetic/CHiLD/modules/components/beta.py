...
class BetaVAE_MLP(nn.Module):
    ...
    def __init__(self, input_dim=3, hidden_dim=128, extra_obs=1, z_dim_list=[1, 2, 4]):
        super(BetaVAE_MLP, self).__init__()
        self.all_z_dim = sum(z_dim_list)
        self.input_dim = input_dim
        self.encoders = nn.ModuleList()
        self.encoders.append(nn.Sequential(
                                    nn.Linear(input_dim * extra_obs, hidden_dim),
                                    nn.LeakyReLU(0.2),
                                    ...
                                    nn.Linear(hidden_dim, hidden_dim),
                                    nn.LeakyReLU(0.2),
                                    nn.Linear(hidden_dim, 2*z_dim_list[-1])
                                    ))
        for idx, z_dim in enumerate(z_dim_list[::-1][1:]):
            self.encoders.append(nn.Sequential(
                                    nn.Linear(z_dim_list[::-1][idx] * 2, hidden_dim),
                                    nn.LeakyReLU(0.2),
                                    nn.Linear(hidden_dim, hidden_dim),
                                    nn.LeakyReLU(0.2),
                                    nn.Linear(hidden_dim, hidden_dim),
        ...
        # self.encoder = nn.Sequential(nn.Linear(input_dim * extra_obs, hidden_dim),
        #                              nn.LeakyReLU(0.2),
        #                              nn.Linear(hidden_dim, hidden_dim),
        #                              nn.LeakyReLU(0.2),
        #                              nn.Linear(hidden_dim, 2*self.all_z_dim))
        # Fix the functional form to ground-truth mixing function
        self.decoder = nn.Sequential(  nn.LeakyReLU(0.2),
                                       nn.Linear(self.all_z_dim, hidden_dim),
                                       nn.LeakyReLU(0.2),
                                       nn.Linear(hidden_dim, hidden_dim),
                                       nn.LeakyReLU(0.2),
                                       nn.Linear(hidden_dim, input_dim)
                                    )
        self.decoder2 = nn.Sequential(  nn.LeakyReLU(0.2),
    ...
    def forward(self, x, return_z=True):
        distributions = self._encode(x)
        mu = distributions[:, :self.all_z_dim]
        logvar = distributions[:, self.all_z_dim:]
        z = reparametrize(mu, logvar)
        x_recon = self._decode(z)
        
        # print(x_recon[0].shape)
        # print(x_recon[1].shape)
        # exit()
    ...
    def _encode(self, x):
        ...
        mus_halves = [z[:, :z.shape[-1]//2] for z in z_list]
        var_halves = [z[:, z.shape[-1]//2:] for z in z_list]
        mus_concat = torch.cat(mus_halves, dim=-1)
        var_concat = torch.cat(var_halves, dim=-1)
        
        return torch.cat((mus_concat, var_concat), dim=-1)
        
        #return self.encoder(x)

    def _decode(self, z):
        z1 = z[:,1:]     
        z2 = torch.unsqueeze(z[:, 0], dim=-1)
        return self.decoder(z)
