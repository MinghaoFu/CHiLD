...
class InstantaneousProcess(pl.LightningModule):
    ...
    def loss_function(self, x, x_recon, mus, logvars, zs, embeddings, x_recon2):
        '''
        ...
        # recon_loss += self.reconstruction_loss(x[:,:self.lag], x_recon2[:,:self.lag], self.decoder_dist) 
    ...
    def forward(self, batch):
        ...
        x_recon = x_recon[0].view(batch_size, length, self.input_dim)
        x_recon2 = x_recon[1].view(batch_size, length, self.input_dim)
        ...
        return x, x_recon, mus, logvars, zs, embeddings, x_recon2

    def training_step(self, batch, batch_idx):
        x, x_recon, mus, logvars, zs, embeddings, x_recon2 = self.forward(batch)
        sparsity_loss, recon_loss, kld_normal, kld_future = self.loss_function(x, x_recon, mus, logvars, zs, embeddings, x_recon2)
